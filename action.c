...
/*   Updated: 2025/08/06 14:48:40 by nlienard         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_monitoring(t_args *lc_args, struct timeval tv, int count)
{
	...
	if (lc_args->tab_philo[lc_args->i][1] == true
		&& lc_args->tab_philo[lc_args->i + 1][1] == true)
	{
		take_fork(lc_args, tv, lc_args->tab_philo[lc_args->i][0]);
		lc_args->tab_philo[lc_args->i][1] = false;
	...
	if (lc_args->tab_philo[lc_args->i + 1][1] == true)
	{
		take_fork(lc_args, tv, lc_args->tab_philo[lc_args->i + 1][0]);
		lc_args->tab_philo[lc_args->i][1] = false;
		return (0);
	}
	return (1);
